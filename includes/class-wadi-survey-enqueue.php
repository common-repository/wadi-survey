...
class WadiSurveyEnqueue
{
    ...
    public function survey_scripts_init()
    {
        wp_enqueue_script(
            'survey_script_front',
            plugins_url( 'assets/dist/survey.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.0.0',
            true
        );
        wp_enqueue_style(
            'survey_styles',
            plugins_url( 'assets/dist/survey.css', realpath( __DIR__ ) ),
            false,
            '1.0.0',
            'all'
        );
        wp_enqueue_script(
            'survey_script_front_multistep',
            plugins_url( 'assets/dist/multistep-survey.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.0.0',
            true
        );
        wp_enqueue_style(
            'survey_multistep_styles',
            plugins_url( 'assets/dist/multistep-survey.css', realpath( __DIR__ ) ),
            false,
            '1.0.0',
            'all'
        );
        wp_localize_script( 'survey-js', 'my_ajax_object', array(
            'ajaxurl' => admin_url( 'admin-ajax.php' ),
        ) );
        // Poll
        wp_enqueue_script(
            'poll_script_front',
            plugins_url( 'assets/dist/poll.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.0.0',
            true
        );
        wp_enqueue_style(
            'poll_styles',
            plugins_url( 'assets/dist/poll.css', realpath( __DIR__ ) ),
            false,
            '1.0.0',
            'all'
        );
    ...
    public function survey_backend_init( $hook )
    {
        ...
        wp_enqueue_style(
            'survey_styles',
            plugins_url( 'assets/dist/main.css', realpath( __DIR__ ) ),
            false,
            '1.0.0',
            'all'
        );
        wp_enqueue_script(
            'survey_js_backend',
            plugins_url( 'assets/dist/main.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.0.0',
            true
        );
    ...
    public function survey_backend_init_single( $hook )
    {
        ...
        wp_enqueue_script(
            'jquery_datatable_wadi',
            plugins_url( 'assets/scripts/jquery.dataTables.min.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.11.3',
            true
        );
        wp_enqueue_script(
            'datatable_bootstrap_wadi',
            plugins_url( 'assets/scripts/dataTables.bootstrap5.min.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.11.3',
            true
        );
    ...
    public function poll_backend_init_single( $hook )
    {
        ...
        wp_enqueue_style(
            'poll_single_styles',
            plugins_url( 'assets/dist/poll-admin.css', realpath( __DIR__ ) ),
            false,
            '1.0.0',
            'all'
        );
        wp_enqueue_script(
            'poll_single_js_backend',
            plugins_url( 'assets/dist/poll-admin.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            '1.0.0',
            true
        );
    ...
    public function poll_tooltip_fn( $hook )
    {
        ...
        wp_enqueue_script(
            'poll_js_tooltip',
            plugins_url( 'assets/dist/wadi-tooltip.js', realpath( __DIR__ ) ),
            array( 'jquery' ),
            time(),
            true
        );
        wp_enqueue_style(
            'poll_css_tooltip',
            plugins_url( 'assets/dist/wadi-tooltip.css', realpath( __DIR__ ) ),
            false,
            time(),
            'all'
        );
