...
if ( function_exists( 'ws_fs' ) ) {
    ws_fs()->set_basename( false, __FILE__ );
} else {
     ...
     * Define Paths
     */
    define( 'WASU_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
    /**
     * Register Task Post Type
     */
    require_once WASU_PLUGIN_PATH . 'includes/class-posttypes.php';
    /**
     * Add in custom fields.
     */
    require_once WASU_PLUGIN_PATH . 'includes/class-wadi-backend.php';
    /**
     * Add Survey Shortcode
     */
    require_once WASU_PLUGIN_PATH . 'includes/survey-shortcode.php';
    /**
     * Add Poll Shortcode
     */
    require_once WASU_PLUGIN_PATH . 'includes/poll-shortcode.php';
    /**
     * Add Survey Database Table
     */
    require_once WASU_PLUGIN_PATH . 'includes/class-survey-db.php';
    /**
     * Add Survey submenu page
     */
    require_once WASU_PLUGIN_PATH . 'includes/class-wadi-admin-menus.php';
    /**
     * Enqueue Wadi Survey Scripts and Styles
     */
    require_once WASU_PLUGIN_PATH . 'includes/class-wadi-survey-enqueue.php';
    /**
     * Wadi Survey Uninstall
     */
    // require_once WASU_PLUGIN_PATH . 'includes/class-wadi-survey-uninstall.php';
    /**
     * Disable Gutenberg on Survey Custom Post Type
     */
    add_filter(
        'use_block_editor_for_post_type',
        'survey_disable_gutenberg',
        10,
        2
    );
    ...
    add_action(
        'manage_wadi-survey_posts_custom_column',
        'survey_shortcode_column_head_content',
        10,
        2
    );
    ...
    add_action(
        'manage_wadi-poll_posts_custom_column',
        'poll_shortcode_column_head_content',
        10,
        2
    );
    ...
    {
        global  $post ;
        $file = WASU_PLUGIN_PATH . 'single-' . $post->post_type . '.php';
        if ( file_exists( $file ) ) {
    ...
    {
        global  $wpdb ;
        $tableArray = [ $wpdb->prefix . 'wadi_survey_submissions', $wpdb->prefix . 'wadi_poll_submissions' ];
        foreach ( $tableArray as $tablename ) {
            $wpdb->query( "DROP TABLE IF EXISTS {$tablename}" );
        }
        // Delete all Wadi Survey Post on install
        $surveyPosts = get_posts( array(
            'post_type'   => 'wadi-survey',
            'numberposts' => -1,
        ) );
        foreach ( $surveyPosts as $surveyPost ) {
            wp_delete_post( $surveyPost->ID, true );
        }
        $pollPosts = get_posts( array(
            'post_type'   => 'wadi-poll',
            'numberposts' => -1,
        ) );
